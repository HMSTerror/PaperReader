...
- 论文 PDF: [Multi-level Cross-view Contrastive Learning for Knowledge-aware Recommender System.pdf](Multi-level Cross-view Contrastive Learning for Knowledge-aware Recommender System.pdf)
- 下载来源: https://arxiv.org/pdf/2204.08807v1.pdf
- 元数据来源: OpenAlex http://arxiv.org/abs/2204.08807v1
- 年份/来源: 2022 / arXiv
- 作者: Ding Zou、Wei Wei、Xian-Ling Mao 等
...
**Materials**: 可提取文本中识别到的数据集或材料包括：MovieLens、Book-Crossing。可提取指标包括：Recall、HR、AUC、MAP。可提取基线包括：BPR、MF、LightGCN、DeepFM、DIN、DIEN。这些信息用于构造训练集、验证集和测试集，并评估模型在候选排序任务上的表现。
...
关于不足和未来方向，本报告只记录可从论文可提取文本中直接看到的作者表述，避免替作者补造局限。可提取线索如下：odel multiple patterns of connections among user, item, and entity for recommendation, which can be further categorized into two classes, namely, path-based [16, 31, 45] and graph ；KPRN [45] further automatically extracts paths between users and items, and utilizes RNNs to model these paths.；, u-r-i-r-e, i-r-e-r-i, etc), further stacking more layers only introduces more noise.
