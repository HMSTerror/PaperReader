...
- 论文 PDF: [KGAT - Knowledge Graph Attention Network for Recommendation.pdf](KGAT - Knowledge Graph Attention Network for Recommendation.pdf)
- 下载来源: https://arxiv.org/pdf/1905.07854v2.pdf
- 元数据来源: OpenAlex http://arxiv.org/abs/1905.07854v2
- 年份/来源: 2019 / arXiv
- 作者: Xiang Wang、Xiangnan He、Yixin Cao 等
...
**Materials**: 可提取文本中识别到的数据集或材料包括：Amazon、Yelp。可提取指标包括：Recall、NDCG、HR、MAP、RMSE。可提取基线包括：BPR、DeepFM、Wide & Deep、DIN、DIEN。这些信息用于构造训练集、验证集和测试集，并评估模型在候选排序任务上的表现。
...
关于不足和未来方向，本报告只记录可从论文可提取文本中直接看到的作者表述，避免替作者补造局限。可提取线索如下：To address the limitation of feature-based SL models, a solution is to take the graph of item side information, aka.；Note that, we employ only inner product on these representations for simplicity, and leave the further exploration of the attention module as the future work.；It further contributes to the representation of entity h at layer l.
