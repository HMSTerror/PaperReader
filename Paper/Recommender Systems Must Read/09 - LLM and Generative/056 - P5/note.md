...
- 论文 PDF: [Recommendation as Language Processing (RLP) - A Unified Pretrain, Personalized Prompt & Predict Paradigm (P5).pdf](Recommendation as Language Processing (RLP) - A Unified Pretrain, Personalized Prompt & Predict Paradigm (P5).pdf)
- 下载来源: https://arxiv.org/pdf/2203.13366v7.pdf
- 元数据来源: OpenAlex http://arxiv.org/abs/2203.13366v7
- 年份/来源: 2022 / arXiv
- 作者: Shijie Geng、Shuchang Liu、Zuohui Fu 等
...
**Materials**: 可提取文本中识别到的数据集或材料包括：Amazon、Yelp。可提取指标包括：NDCG、HR、RMSE。可提取基线包括：BPR、MF、GRU4Rec、SASRec、Caser、DIN。这些信息用于构造训练集、验证集和测试集，并评估模型在候选排序任务上的表现。
...
关于不足和未来方向，本报告只记录可从论文可提取文本中直接看到的作者表述，避免替作者补造局限。可提取线索如下：Later on, the contextual features such as user profile and item metadata are further inte- grated into the system through more sophisticated models such as factorization machines [；ce describing the following product review from  Mom of 3 yo girl: First it came with the packaging open and then as soon as my son  took it out it was so easily broken.；One limitation of these methods is that they still require additional finetuning on downstream datasets.
