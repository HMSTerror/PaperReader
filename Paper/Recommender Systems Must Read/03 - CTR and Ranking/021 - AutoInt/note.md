...
- 论文 PDF: [AutoInt - Automatic Feature Interaction Learning via Self-Attentive Neural Networks.pdf](AutoInt - Automatic Feature Interaction Learning via Self-Attentive Neural Networks.pdf)
- 下载来源: https://arxiv.org/pdf/1810.11921v2.pdf
- 元数据来源: OpenAlex http://arxiv.org/abs/1810.11921v2
- 年份/来源: 2018 / arXiv
- 作者: Weiping Song、Chence Shi、Zhiping Xiao 等
...
**Materials**: 可提取文本中识别到的数据集或材料包括：MovieLens、Criteo、Avazu。可提取指标包括：HR、AUC、Logloss、MAP、RMSE。可提取基线包括：DeepFM、Wide & Deep、DIN、DIEN。这些信息用于构造训练集、验证集和测试集，并评估模型在候选排序任务上的表现。
...
关于不足和未来方向，本报告只记录可从论文可提取文本中直接看到的作者表述，避免替作者补造局限。可提取线索如下：However, such kinds of methods suffer from two limitations.；We conclude this paper and point out the future work in Section 6.；[36] further proposed multi-head self-attention to model complicated dependencies be- tween words in machine translation.
