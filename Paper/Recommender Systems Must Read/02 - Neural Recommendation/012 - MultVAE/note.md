...
- 论文 PDF: [Variational Autoencoders for Collaborative Filtering.pdf](Variational Autoencoders for Collaborative Filtering.pdf)
- 下载来源: https://arxiv.org/pdf/1802.05814v1.pdf
- 元数据来源: OpenAlex http://arxiv.org/abs/1802.05814v1
- 年份/来源: 2018 / arXiv
- 作者: Dawen Liang、Rahul G. Krishnan、Matthew D. Hoffman 等
...
**Materials**: 可提取文本中识别到的数据集或材料包括：MovieLens、Netflix、Pinterest。可提取指标包括：Recall、NDCG、HR、Precision。可提取基线包括：BPR、MF、NCF、AutoRec、DIN、DIEN。这些信息用于构造训练集、验证集和测试集，并评估模型在候选排序任务上的表现。
...
关于不足和未来方向，本报告只记录可从论文可提取文本中直接看到的作者表述，避免替作者补造局限。可提取线索如下：We extend variational autoencoders (VAEs) to collaborative filtering for implicit feedback.；com ABSTRACT We extend variational autoencoders (vaes) to collaborative filtering for implicit feedback.；Here, we extend variational autoencoders (vaes) [24, 37] to col- laborative filtering for implicit feedback.
