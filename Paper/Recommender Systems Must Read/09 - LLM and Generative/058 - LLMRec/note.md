...
- 论文 PDF: [LLMRec - Large Language Models with Graph Augmentation for Recommendation.pdf](LLMRec - Large Language Models with Graph Augmentation for Recommendation.pdf)
- 下载来源: https://arxiv.org/pdf/2311.00423.pdf
- 元数据来源: OpenAlex https://openalex.org/W4388275974
- 年份/来源: 2023 / arXiv (Cornell University)
- 作者: Wei Wei、Xubin Ren、Jiabin Tang 等

## 摘要

本文是 **大语言模型推荐 (LLM-based Recommendation) 与生成式推荐 (Generative Recommendation)** 方向的重要论文。论文围绕推荐系统中“如何从稀疏、噪声化、动态变化的用户行为中学习可靠排序信号”这一核心问题展开，试图用新的模型结构或训练目标改善传统协同过滤与深度推荐的不足。OpenAlex 记录的被引次数约为 10 次，说明其在推荐系统文献中具有较高影响力。 对初学者而言，这篇文章的价值不只在于提出一个模型名称，更在于它明确回答了推荐系统研究中一个常见问题：模型应当学习什么形式的用户偏好，以及这种偏好如何转化为可评测的 Top-N 推荐结果。
...
**Materials**: 可提取文本中识别到的数据集或材料包括：MovieLens、Netflix。可提取指标包括：Recall、NDCG、HR、MAP、Precision。可提取基线包括：BPR、MF、LightGCN、NGCF、DIN、DIEN。这些信息用于构造训练集、验证集和测试集，并评估模型在候选排序任务上的表现。
...
关于不足和未来方向，本报告只记录可从论文可提取文本中直接看到的作者表述，避免替作者补造局限。可提取线索如下：udies pro- vide valuable insights into the impact of our LLM-enhanced data augmentation strategies, further solidifying the model efficacy.；ummarize user profiles by utilizing users’ historical interactions and item information to overcome limitation of privacy.；{3, 10, 30} are explored due to cost limitations, and Table 5 shows that C = 10 yields the best results.
