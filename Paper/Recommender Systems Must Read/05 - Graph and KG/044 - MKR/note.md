...
- 论文 PDF: [Multi-Task Feature Learning for Knowledge Graph Enhanced Recommendation.pdf](Multi-Task Feature Learning for Knowledge Graph Enhanced Recommendation.pdf)
- 下载来源: https://arxiv.org/pdf/1901.08907v1.pdf
- 元数据来源: OpenAlex http://arxiv.org/abs/1901.08907v1
- 年份/来源: 2019 / arXiv
- 作者: Hongwei Wang、Fuzheng Zhang、Miao Zhao 等
...
**Materials**: 可提取文本中识别到的数据集或材料包括：MovieLens、Book-Crossing。可提取指标包括：Recall、HR、AUC、MAP、Precision、RMSE。可提取基线包括：DeepFM、DIN、DIEN。这些信息用于构造训练集、验证集和测试集，并评估模型在候选排序任务上的表现。
...
关于不足和未来方向，本报告只记录可从论文可提取文本中直接看到的作者表述，避免替作者补造局限。可提取线索如下：The proposed approach To address the limitations of previous work, we propose MKR, a multi-task learning (MTL) approach for knowledge graph enhanced recommendation.；We will further validate the similarity in the experiments section.；ploring a more elaborate design of layers in the recommendation module is an important direction of future work.
