...
- 论文 PDF: [Neural Attentive Session-based Recommendation.pdf](Neural Attentive Session-based Recommendation.pdf)
- 下载来源: https://arxiv.org/pdf/1711.04725v1.pdf
- 元数据来源: OpenAlex http://arxiv.org/abs/1711.04725v1
- 年份/来源: 2017 / arXiv
- 作者: Jing Li、Pengjie Ren、Zhumin Chen 等
...
**Materials**: 可提取文本中识别到的数据集或材料包括：Amazon、Diginetica、Yoochoose。可提取指标包括：Recall、HR、MRR、MAP。可提取基线包括：BPR、MF、AutoRec、DIN、DIEN。这些信息用于构造训练集、验证集和测试集，并评估模型在候选排序任务上的表现。
...
关于不足和未来方向，本报告只记录可从论文可提取文本中直接看到的作者表述，避免替作者补造局限。可提取线索如下：[40] further im prove this RNN-based model by utilizing two crucial techniques, i .；[40] further study the applica tion of RNN in session-based recommendation.；5 ANALYSIS In this section, We further explore the inﬂuences of using diﬀerent session features in NARM and analyze the eﬀectiveness of the adopted attention mechanism.
