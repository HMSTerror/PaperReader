...
- 论文 PDF: [RippleNet - Propagating User Preferences on the Knowledge Graph for Recommender Systems.pdf](RippleNet - Propagating User Preferences on the Knowledge Graph for Recommender Systems.pdf)
- 下载来源: https://arxiv.org/pdf/1803.03467v4.pdf
- 元数据来源: OpenAlex http://arxiv.org/abs/1803.03467v4
- 年份/来源: 2018 / arXiv
- 作者: Hongwei Wang、Fuzheng Zhang、Jialin Wang 等
...
**Materials**: 可提取文本中识别到的数据集或材料包括：MovieLens、Book-Crossing。可提取指标包括：Recall、HR、AUC、MAP、Precision、RMSE。可提取基线包括：Wide & Deep、DIN、DIEN。这些信息用于构造训练集、验证集和测试集，并评估模型在候选排序任务上的表现。
...
关于不足和未来方向，本报告只记录可从论文可提取文本中直接看到的作者表述，避免替作者补造局限。可提取线索如下：To address the limitations of existing methods, we propose Rip- pleNet, an end-to-end framework for knowledge-graph-aware rec- ommendation.；" (country) and "Drama" (genre), while "Tom Hanks" is further linked with films "The Terminal" and "Cast Away" which he starred in.；(4) In RippleNet, we can sample a fixed-size set of neighbors instead of using a full ripple set to further reduce the computation overhead.
