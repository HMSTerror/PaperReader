...
- 论文 PDF: [Knowledge Graph Contrastive Learning for Recommendation.pdf](Knowledge Graph Contrastive Learning for Recommendation.pdf)
- 下载来源: https://arxiv.org/pdf/2205.00976v2.pdf
- 元数据来源: OpenAlex http://arxiv.org/abs/2205.00976v2
- 年份/来源: 2022 / arXiv
- 作者: Yuhao Yang、Chao Huang、Lianghao Xia 等
...
**Materials**: 可提取文本中识别到的数据集或材料包括：Amazon、Yelp。可提取指标包括：Recall、NDCG、HR、MAP。可提取基线包括：BPR、MF、NCF、AutoRec、LightGCN、DIN、DIEN。这些信息用于构造训练集、验证集和测试集，并评估模型在候选排序任务上的表现。
...
关于不足和未来方向，本报告只记录可从论文可提取文本中直接看到的作者表述，避免替作者补造局限。可提取线索如下：learning paradigm, giving a greater role to unbiased user-item interactions in gradient descent and further suppressing the noise.；arning paradigm, giving a greater role to unbiased user-item interactions in gradient de- scent and further suppressing the noise.；In light of the aforementioned limitations and chal- lenges, we propose a generalKnowledge Graph Contrastive Learning framework (KGCL) for recommendation.
