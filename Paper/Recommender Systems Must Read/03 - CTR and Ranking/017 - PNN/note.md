...
- 论文 PDF: [Product-based Neural Networks for User Response Prediction.pdf](Product-based Neural Networks for User Response Prediction.pdf)
- 下载来源: https://arxiv.org/pdf/1611.00144v1.pdf
- 元数据来源: OpenAlex http://arxiv.org/abs/1611.00144v1
- 年份/来源: 2016 / arXiv
- 作者: Yanru Qu、Han Cai、Kan Ren 等
...
**Materials**: 可提取文本中识别到的数据集或材料包括：Criteo。可提取指标包括：Recall、HR、AUC、MAP、RMSE。可提取基线包括：DIN、DIEN。这些信息用于构造训练集、验证集和测试集，并评估模型在候选排序任务上的表现。
...
关于不足和未来方向，本报告只记录可从论文可提取文本中直接看到的作者表述，避免替作者补造局限。可提取线索如下：tegorical data, a product layer to capture interactive patterns between inter-field categories, and further fully connected layers to explore high-order feature interactions.；tegorical data, a product layer to capture interactive patterns between inter- ﬁeld categories, and further fully connected layers to explore high-order feature interactions.；layer based on the embedded feature vectors to model the inter-ﬁeld feature interactions, and (iii) further distills the high-order feature patterns with fully connected MLPs.
