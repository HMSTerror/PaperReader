...
- 论文 PDF: [Are We Really Making Much Progress - A Worrying Analysis of Recent Neural Recommendation Approaches.pdf](Are We Really Making Much Progress - A Worrying Analysis of Recent Neural Recommendation Approaches.pdf)
- 下载来源: https://arxiv.org/pdf/1907.06902v3.pdf
- 元数据来源: OpenAlex http://arxiv.org/abs/1907.06902v3
- 年份/来源: 2019 / arXiv
...
**Materials**: 可提取文本中识别到的数据集或材料包括：MovieLens、Amazon、Yelp、LastFM、Pinterest。可提取指标包括：Recall、NDCG、Hit Rate、HR、MRR、MAP、Precision。可提取基线包括：MF、NCF、DIN。这些信息用于构造训练集、验证集和测试集，并评估模型在候选排序任务上的表现。
...
关于不足和未来方向，本报告只记录可从论文可提取文本中直接看到的作者表述，避免替作者补造局限。可提取线索如下：4We will re-run parameter optimization for the reproduced al gorithms as part of our future work in order to validate the parameter optimization procedures used by the authors.；In our ongoing and future work, we plan to extend our analysis to other publication outlets and other t ypes of recommendation problems.
