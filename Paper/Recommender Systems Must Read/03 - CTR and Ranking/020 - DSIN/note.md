...
- 论文 PDF: [Deep Session Interest Network for Click-Through Rate Prediction.pdf](Deep Session Interest Network for Click-Through Rate Prediction.pdf)
- 下载来源: https://arxiv.org/pdf/1905.06482v1.pdf
- 元数据来源: OpenAlex http://arxiv.org/abs/1905.06482v1
- 年份/来源: 2019 / arXiv
- 作者: Yufei Feng、Fuyu Lv、Weichen Shen 等
...
**Materials**: 可提取文本中识别到的数据集或材料包括：Amazon、Taobao。可提取指标包括：HR、AUC、RMSE。可提取基线包括：DeepFM、Wide & Deep、DIN、DIEN。这些信息用于构造训练集、验证集和测试集，并评估模型在候选排序任务上的表现。
...
关于不足和未来方向，本报告只记录可从论文可提取文本中直接看到的作者表述，避免替作者补造局限。可提取线索如下：We further discuss the effectiveness of critical technical designs in DSIN empirically.；It will lead to information loss in the process of information transmis- sion in RNNs and further confuse the representation of users’ behavior sequences.；In the future, we will pay attention to utilizing knowledge graph as prior knowledge to explain users’ historical behaviors for bet- ter CTR prediction.
