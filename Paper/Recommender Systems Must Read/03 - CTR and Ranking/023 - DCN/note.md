...
- 论文 PDF: [Deep & Cross Network for Ad Click Predictions.pdf](Deep & Cross Network for Ad Click Predictions.pdf)
- 下载来源: https://arxiv.org/pdf/1708.05123v1.pdf
- 元数据来源: OpenAlex http://arxiv.org/abs/1708.05123v1
- 年份/来源: 2017 / arXiv
- 作者: Ruoxi Wang、Bin Fu、Gang Fu 等
...
**Materials**: 可提取文本中识别到的数据集或材料包括：Criteo。可提取指标包括：HR、Logloss、MAP、RMSE。可提取基线包括：Wide & Deep、DIN、DIEN。这些信息用于构造训练集、验证集和测试集，并评估模型在候选排序任务上的表现。
...
关于不足和未来方向，本报告只记录可从论文可提取文本中直接看到的作者表述，避免替作者补造局限。可提取线索如下：Field-aware factorization mac hines (FFMs) [7, 8] further allow each feature to learn several vec tors where each vector is associated with a ﬁeld.；eneralization of FMs /T_he cross network shares the spirit of parameter sharing as the FM model and further extends it to a deeper structure.；of diﬀeren t mod- els in logloss, then we compare DCN with DNN in detail, that is , we investigate further into the eﬀects introduced by the cro ss net- work.
