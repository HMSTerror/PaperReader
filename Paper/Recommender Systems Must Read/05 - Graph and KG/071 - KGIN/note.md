...
- 论文 PDF: [Learning Intents behind Interactions with Knowledge Graph for Recommendation.pdf](Learning Intents behind Interactions with Knowledge Graph for Recommendation.pdf)
- 下载来源: https://arxiv.org/pdf/2102.07057v1.pdf
- 元数据来源: OpenAlex http://arxiv.org/abs/2102.07057v1
- 年份/来源: 2021 / arXiv
- 作者: Xiang Wang、Tinglin Huang、Dingxian Wang 等
...
**Materials**: 可提取文本中识别到的数据集或材料包括：Amazon。可提取指标包括：Recall、NDCG、HR、MAP。可提取基线包括：BPR、MF、LightGCN、DIN、DIEN。这些信息用于构造训练集、验证集和测试集，并评估模型在候选排序任务上的表现。
...
关于不足和未来方向，本报告只记录可从论文可提取文本中直接看到的作者表述，避免替作者补造局限。可提取线索如下：Further analyses show that KGIN offers interpretable explanations for predictions by identifying influential intents and relational paths.；Further analyses show that KGIN offers interpretable explanations for predictions by identifying influential intents and relational paths.；nowledge Graph-based Intent Network (KGIN), which consists of two components to solve the foregoing limitations correspondingly: (1) User Intent Modeling.
