...
- 论文 PDF: [Sparse-Interest Network for Sequential Recommendation.pdf](Sparse-Interest Network for Sequential Recommendation.pdf)
- 下载来源: https://arxiv.org/pdf/2102.09267v1.pdf
- 元数据来源: OpenAlex http://arxiv.org/abs/2102.09267v1
- 年份/来源: 2021 / arXiv
- 作者: Qiaoyu Tan、Jianwei Zhang、Jiangchao Yao 等
...
**Materials**: 可提取文本中识别到的数据集或材料包括：MovieLens、Amazon、Taobao、Tmall。可提取指标包括：Recall、NDCG、Hit Rate、HR、MAP。可提取基线包括：MF、GRU4Rec、SASRec、NCF、AutoRec、Caser、LightGCN、DeepFM、DIN、PinSage。这些信息用于构造训练集、验证集和测试集，并评估模型在候选排序任务上的表现。
...
关于不足和未来方向，本报告只记录可从论文可提取文本中直接看到的作者表述，避免替作者补造局限。可提取线索如下：To relax this limitation, there are also methods adopting high-order MCs that consider more previous items [11, 12, 45].；Results on the billion-scale industrial dataset further confirm our model’s effectiveness in terms of rec- ommendation accuracy and producing reasonable item clusters.
