...
- 论文 PDF: [Controllable Multi-Interest Framework for Recommendation.pdf](Controllable Multi-Interest Framework for Recommendation.pdf)
- 下载来源: https://arxiv.org/pdf/2005.09347v2.pdf
- 元数据来源: OpenAlex http://arxiv.org/abs/2005.09347v2
- 年份/来源: 2020 / arXiv
- 作者: Yukuo Cen、Jianwei Zhang、Xu Zou 等
...
**Materials**: 可提取文本中识别到的数据集或材料包括：Amazon、Taobao、Tmall。可提取指标包括：Recall、NDCG、Hit Rate、HR、MAP。可提取基线包括：MF、GRU4Rec、SASRec、NCF、DeepFM、DIN、DIEN、PinSage。这些信息用于构造训练集、验证集和测试集，并评估模型在候选排序任务上的表现。
...
关于不足和未来方向，本报告只记录可从论文可提取文本中直接看到的作者表述，避免替作者补造局限。可提取线索如下：Results on the billion-scale in- dustrial dataset further confirm the effectiveness and efficiency of our model in practice.；We extend the w2 into a da-by-K matrix as W2.；4 Industrial Results We further experiment on the industrial dataset collected by Mobile Taobao App on February 8th, 2020.
