...
- 论文 PDF: [CORE - Simple and Effective Session-based Recommendation within Consistent Representation Space.pdf](CORE - Simple and Effective Session-based Recommendation within Consistent Representation Space.pdf)
- 下载来源: https://arxiv.org/pdf/2204.11067v1.pdf
- 元数据来源: OpenAlex http://arxiv.org/abs/2204.11067v1
- 年份/来源: 2022 / arXiv
- 作者: Yupeng Hou、Binbin Hu、Zhiqiang Zhang 等

## 摘要

本文是 **序列推荐 (Sequential Recommendation) 与会话推荐 (Session-based Recommendation)** 方向的重要论文。论文围绕推荐系统中“如何从稀疏、噪声化、动态变化的用户行为中学习可靠排序信号”这一核心问题展开，试图用新的模型结构或训练目标改善传统协同过滤与深度推荐的不足。OpenAlex 未返回稳定被引次数。 对初学者而言，这篇文章的价值不只在于提出一个模型名称，更在于它明确回答了推荐系统研究中一个常见问题：模型应当学习什么形式的用户偏好，以及这种偏好如何转化为可评测的 Top-N 推荐结果。
...
**Materials**: 可提取文本中识别到的数据集或材料包括：Tmall、RetailRocket、Diginetica、Yoochoose。可提取指标包括：Recall、HR。可提取基线包括：GRU4Rec、SASRec、DIN。这些信息用于构造训练集、验证集和测试集，并评估模型在候选排序任务上的表现。
...
关于不足和未来方向，本报告只记录可从论文可提取文本中直接看到的作者表述，避免替作者补造局限。可提取线索如下：plementation details The proposed models and all the base- lines are implemented based on a popular open-source recommen- dation library RecBole1 [32] and its extension RecBole-GNN；Without carefully designed encoder architecture, CORE can achieve impressive performance, which further confirms the importance of encoding session embed- dings and item embeddings
