...
- 论文 PDF: [Personalized Top-N Sequential Recommendation via Convolutional Sequence Embedding.pdf](Personalized Top-N Sequential Recommendation via Convolutional Sequence Embedding.pdf)
- 下载来源: https://arxiv.org/pdf/1809.07426v1.pdf
- 元数据来源: OpenAlex http://arxiv.org/abs/1809.07426v1
- 年份/来源: 2018 / arXiv
...
**Materials**: 可提取文本中识别到的数据集或材料包括：MovieLens、Amazon、Gowalla、Tmall。可提取指标包括：Recall、HR、MAP、Precision。可提取基线包括：BPR、MF、GRU4Rec、AutoRec、Caser、DIN、DIEN。这些信息用于构造训练集、验证集和测试集，并评估模型在候选排序任务上的表现。
...
关于不足和未来方向，本报告只记录可从论文可提取文本中直接看到的作者表述，避免替作者补造局限。可提取线索如下：ed in the past and aims to predict top-$N$ ranked items that a user will likely interact in a `near future'.；ted in the past and aims to predict top- N ranked items that a user will likely interact in a “near future”.；top-N sequential recommendation that recommends N items that a user likely interacts with in a near future.
