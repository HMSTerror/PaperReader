...
- 论文 PDF: [Mining Latent Structures for Multimedia Recommendation.pdf](Mining Latent Structures for Multimedia Recommendation.pdf)
- 下载来源: https://arxiv.org/pdf/2104.09036.pdf
- 元数据来源: OpenAlex https://openalex.org/W3192113933
- 年份/来源: 2021 / 出版源未在 OpenAlex 主位置中完整给出
- 作者: Jinghao Zhang、Yanqiao Zhu、Qiang Liu 等

## 摘要

本文是 **多模态推荐 (Multimodal Recommendation)** 方向的重要论文。论文围绕推荐系统中“如何从稀疏、噪声化、动态变化的用户行为中学习可靠排序信号”这一核心问题展开，试图用新的模型结构或训练目标改善传统协同过滤与深度推荐的不足。OpenAlex 记录的被引次数约为 307 次，说明其在推荐系统文献中具有较高影响力。 对初学者而言，这篇文章的价值不只在于提出一个模型名称，更在于它明确回答了推荐系统研究中一个常见问题：模型应当学习什么形式的用户偏好，以及这种偏好如何转化为可评测的 Top-N 推荐结果。
...
**Materials**: 可提取文本中识别到的数据集或材料包括：Amazon。可提取指标包括：Recall、NDCG、HR、Precision。可提取基线包括：BPR、MF、LightGCN、NGCF、DIN、DIEN。这些信息用于构造训练集、验证集和测试集，并评估模型在候选排序任务上的表现。
...
关于不足和未来方向，本报告只记录可从论文可提取文本中直接看到的作者表述，避免替作者补造局限。可提取线索如下：derlying these multimodal contents could be beneficial for learning better item representations and further boosting recommendation.；derlying these multimodal contents could be beneficial for learning better item representations and further boosting rec- ommendation.；We further divide the half of the items (10%) into the validation set and half (10%) into the testing set.
