...
原始笔记对该论文方法的具体概括如下：DiffRec 把每个用户的全量 item 交互向量看成一个需要“去噪恢复”的信号。训练时，模型在交互向量上加入受控的高斯噪声，再训练一个 denoising 网络把带噪向量恢复成原始偏好向量。与图像扩散不同，论文刻意减小噪声尺度并减少推理步数，因为推荐任务的目标不是从纯随机噪声生成一张全新图片，而是在保留用户历史个性的前提下修复和补全偏好。
...
原始笔记记录的主要实验结论如下：论文在 Amazon-book、Yelp、ML-1M 三个数据集上做了 clean training、noisy training 和 temporal training 等多种设置，并与 MF、LightGCN、CDAE、MultiDAE、MultiVAE、CODIGEM 等方法比较。clean setting 下，DiffRec 在三组数据的 Recall 和 NDCG 上整体领先，例如 Amazon-book 上 Recall@10 相对最强基线提升 10.67%，NDCG@10 提升 14.76%；Yelp 上提升较小但仍稳定；ML-1M 上 NDCG@10 提升 9.21%。
...
原始笔记对论文原理的直观解释如下：可以把用户历史想成一张被涂脏的“兴趣清单”：里面有真正喜欢的东西，也有误点、偶然购买和过期兴趣。DiffRec 做的事就是反复练习“把脏清单修干净”。训练时它故意往清单里加一点噪声，让模型学会把噪声拿掉；推理时，它把用户当前行为当作一份不完美清单，输出更可能代表真实兴趣的 item 分数。
...
关于不足与未来方向，本笔记只记录作者在论文结论或原始笔记中能够直接支撑的内容。对于《Diffusion Recommender Model》而言，当前可用材料没有提供独立的逐条 Limitation 小节；因此这里不额外推断新的缺点。可以确定的是，作者把后续发展空间放在模型效率、条件信息利用、生成质量和推荐任务适配等方向上，这些方向均服务于更稳定地把扩散式生成机制转化为可排序、可评测、可部署的推荐模型。
