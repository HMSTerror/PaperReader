...
- 论文 PDF: [LightGCL - Simple Yet Effective Graph Contrastive Learning for Recommendation.pdf](LightGCL - Simple Yet Effective Graph Contrastive Learning for Recommendation.pdf)
- 下载来源: https://arxiv.org/pdf/2302.08191v3.pdf
- 元数据来源: OpenAlex http://arxiv.org/abs/2302.08191v3
- 年份/来源: 2023 / arXiv
- 作者: Xuheng Cai、Chao Huang、Lianghao Xia 等
...
**Materials**: 可提取文本中识别到的数据集或材料包括：Amazon、Yelp、Gowalla、Tmall。可提取指标包括：Recall、NDCG、HR、MAP。可提取基线包括：BPR、MF、SVD++、NCF、LightGCN、DIN。这些信息用于构造训练集、验证集和测试集，并评估模型在候选排序任务上的表现。
...
关于不足和未来方向，本报告只记录可从论文可提取文本中直接看到的作者表述，避免替作者补造局限。可提取线索如下：Further analyses demonstrate the superiority of LightGCL's robustness against data sparsity and popularity bias.；Further analyses demonstrate the superiority of LightGCL’s robustness against data sparsity and popularity bias.；their effectiveness, state-of-the-art contrastive recommender systems suffer from several inherent limitations: i) Graph augmentation with random perturbation may lose useful struc
