...
- 论文 PDF: [Improving Graph Collaborative Filtering with Neighborhood-enriched Contrastive Learning.pdf](Improving Graph Collaborative Filtering with Neighborhood-enriched Contrastive Learning.pdf)
- 下载来源: https://arxiv.org/pdf/2202.06200v2.pdf
- 元数据来源: OpenAlex http://arxiv.org/abs/2202.06200v2
- 年份/来源: 2022 / arXiv
- 作者: Zihan Lin、Changxin Tian、Yupeng Hou 等
...
**Materials**: 可提取文本中识别到的数据集或材料包括：MovieLens、Amazon、Yelp、Gowalla。可提取指标包括：Recall、NDCG、HR。可提取基线包括：BPR、MF、LightGCN、NGCF、DIN。这些信息用于构造训练集、验证集和测试集，并评估模型在候选排序任务上的表现。
...
关于不足和未来方向，本报告只记录可从论文可提取文本中直接看到的作者表述，避免替作者补造局限。可提取线索如下：Recently, CF is further enhanced by the powerful graph neural networks (GNN) [9, 31], which models the interaction data as graphs (e.；Since the prototype is latent, we further propose to use an expectation-maximization (EM) algorithm [19] to infer the prototypes.；And the readout function 𝑓readout(·) further summarizes all of the representations [𝒛 (0) 𝑢 , 𝒛 (1) 𝑢 , .
