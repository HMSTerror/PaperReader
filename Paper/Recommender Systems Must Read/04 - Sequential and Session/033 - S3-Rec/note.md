...
- 论文 PDF: [S3-Rec - Self-Supervised Learning for Sequential Recommendation with Mutual Information Maximization.pdf](S3-Rec - Self-Supervised Learning for Sequential Recommendation with Mutual Information Maximization.pdf)
- 下载来源: https://arxiv.org/pdf/2008.07873.pdf
- 元数据来源: OpenAlex https://openalex.org/W3065542300
- 年份/来源: 2020 / 出版源未在 OpenAlex 主位置中完整给出
- 作者: Kun Zhou、Hui Wang、Wayne Xin Zhao 等

## 摘要

本文是 **序列推荐 (Sequential Recommendation) 与会话推荐 (Session-based Recommendation)** 方向的重要论文。论文围绕推荐系统中“如何从稀疏、噪声化、动态变化的用户行为中学习可靠排序信号”这一核心问题展开，试图用新的模型结构或训练目标改善传统协同过滤与深度推荐的不足。OpenAlex 记录的被引次数约为 713 次，说明其在推荐系统文献中具有较高影响力。 对初学者而言，这篇文章的价值不只在于提出一个模型名称，更在于它明确回答了推荐系统研究中一个常见问题：模型应当学习什么形式的用户偏好，以及这种偏好如何转化为可评测的 Top-N 推荐结果。
...
**Materials**: 可提取文本中识别到的数据集或材料包括：Amazon、Yelp、LastFM、Taobao。可提取指标包括：NDCG、HR、MRR、MAP。可提取基线包括：GRU4Rec、SASRec、Caser、DIN。这些信息用于构造训练集、验证集和测试集，并评估模型在候选排序任务上的表现。
...
关于不足和未来方向，本报告只记录可从论文可提取文本中直接看到的作者表述，避免替作者补造局限。可提取线索如下：Besides, we extend our self-supervised learning method to other recommendation models, which also im- prove their performance.；A series of works follow this line and extend it for high-order MCs [ 4, 8, 24].；3 Self-supervised Learning with MIM Based on the above self-attention model, we further incorporate additional self-supervised signals with MIM to enhance the repre- sentations of 
