...
- 论文 PDF: [Hypergraph Contrastive Collaborative Filtering.pdf](Hypergraph Contrastive Collaborative Filtering.pdf)
- 下载来源: https://arxiv.org/pdf/2204.12200v2.pdf
- 元数据来源: OpenAlex http://arxiv.org/abs/2204.12200v2
- 年份/来源: 2022 / arXiv
- 作者: Lianghao Xia、Chao Huang、Yong Xu 等
...
**Materials**: 可提取文本中识别到的数据集或材料包括：MovieLens、Amazon、Yelp。可提取指标包括：Recall、NDCG、HR、MAP。可提取基线包括：BPR、MF、NCF、AutoRec、LightGCN、NGCF、DIN、DIEN、PinSage。这些信息用于构造训练集、验证集和测试集，并评估模型在候选排序任务上的表现。
...
关于不足和未来方向，本报告只记录可从论文可提取文本中直接看到的作者表述，避免替作者补造局限。可提取线索如下：th user-item interaction edges to capture CF signals among neighboring nodes in subgraphs, so as to further improve the user representation quality.；models have offered state-of-the-art performance for recommenda- tion applications, two key issues remain less explored: i) Over-Smoothing Collaborative Effects.；Further ablation study is provided to investigate the effect of key modules in HCCF.
