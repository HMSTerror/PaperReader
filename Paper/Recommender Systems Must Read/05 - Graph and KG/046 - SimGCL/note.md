...
- 论文 PDF: [Are Graph Augmentations Necessary - Simple Graph Contrastive Learning for Recommendation.pdf](Are Graph Augmentations Necessary - Simple Graph Contrastive Learning for Recommendation.pdf)
- 下载来源: https://arxiv.org/pdf/2112.08679v4.pdf
- 元数据来源: OpenAlex http://arxiv.org/abs/2112.08679v4
- 年份/来源: 2021 / arXiv
- 作者: Junliang Yu、Hongzhi Yin、Xin Xia 等
...
**Materials**: 可提取文本中识别到的数据集或材料包括：Amazon、Yelp。可提取指标包括：Recall、NDCG、HR、MAP。可提取基线包括：BPR、LightGCN、NGCF、DIN、DIEN。这些信息用于构造训练集、验证集和测试集，并评估模型在候选排序任务上的表现。
...
关于不足和未来方向，本报告只记录可从论文可提取文本中直接看到的作者表述，避免替作者补造局限。可提取线索如下：The message passing mechanism further exacerbates the clustering problem (i.；We first further compare SGL with SimGCL on three different datasets with different layer settings.；A 2-layer setting is used in this part and the other parameters remain unchanged.
