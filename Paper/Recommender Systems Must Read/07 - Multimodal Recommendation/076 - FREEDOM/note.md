...
- 论文 PDF: [A Tale of Two Graphs - Freezing and Denoising Graph Structures for Multimodal Recommendation.pdf](A Tale of Two Graphs - Freezing and Denoising Graph Structures for Multimodal Recommendation.pdf)
- 下载来源: https://arxiv.org/pdf/2211.06924v3.pdf
- 元数据来源: OpenAlex http://arxiv.org/abs/2211.06924v3
- 年份/来源: 2022 / arXiv
- 作者: Xin Zhou、Zhiqi Shen
...
**Materials**: 可提取文本中识别到的数据集或材料包括：Amazon。可提取指标包括：Recall、NDCG、HR。可提取基线包括：BPR、MF、LightGCN、DIN。这些信息用于构造训练集、验证集和测试集，并评估模型在候选排序任务上的表现。
...
关于不足和未来方向，本报告只记录可从论文可提取文本中直接看到的作者表述，避免替作者补造局限。可提取线索如下：We further employ 𝑘NN sparsification [1] and convert the weighted 𝑺𝑚 into an unweighted matrix.；We further investigate the impact of the frozen item-item graph in FREEDOM and the learnable graph in LATTICE on validation accuracy.；However, de- noising the user-item graph can further improve the performance.
