...
- 论文 PDF: [Multi-Interest Network with Dynamic Routing for Recommendation at Tmall.pdf](Multi-Interest Network with Dynamic Routing for Recommendation at Tmall.pdf)
- 下载来源: https://arxiv.org/pdf/1904.08030v1.pdf
- 元数据来源: OpenAlex http://arxiv.org/abs/1904.08030v1
- 年份/来源: 2019 / arXiv
- 作者: Chao Li、Zhiyuan Liu、Mengmeng Wu 等
...
**Materials**: 可提取文本中识别到的数据集或材料包括：Amazon、Tmall。可提取指标包括：Recall、Hit Rate、HR、MAP。可提取基线包括：MF、NCF、DeepFM、DIN。这些信息用于构造训练集、验证集和测试集，并评估模型在候选排序任务上的表现。
...
关于不足和未来方向，本报告只记录可从论文可提取文本中直接看到的作者表述，避免替作者补造局限。可提取线索如下：Each cluster of historical behaviors is further used to infer the user representation vector correspond- ing to one particular interest.；the deployment of MIND in large-scale industrial application; The last section gives conclusion and future work of this paper.；layer are transformed into embeddings through the embedding layer, and embeddings of each item are further averaged by a pooling layer.
