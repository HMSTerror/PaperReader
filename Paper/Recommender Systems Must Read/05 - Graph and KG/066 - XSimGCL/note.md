...
- 论文 PDF: [XSimGCL - Towards Extremely Simple Graph Contrastive Learning for Recommendation.pdf](XSimGCL - Towards Extremely Simple Graph Contrastive Learning for Recommendation.pdf)
- 下载来源: https://arxiv.org/pdf/2209.02544v4.pdf
- 元数据来源: OpenAlex http://arxiv.org/abs/2209.02544v4
- 年份/来源: 2022 / arXiv
- 作者: Junliang Yu、Xin Xia、Tong Chen 等
...
**Materials**: 可提取文本中识别到的数据集或材料包括：Amazon、Yelp。可提取指标包括：Recall、NDCG、HR、MAP。可提取基线包括：BPR、LightGCN、DIN、DIEN。这些信息用于构造训练集、验证集和测试集，并评估模型在候选排序任务上的表现。
...
关于不足和未来方向，本报告只记录可从论文可提取文本中直接看到的作者表述，避免替作者补造局限。可提取线索如下：Considering these limitations, it may be more practical to pursue alternative augmentations that are both more effective and efficient.；To further investigate this, in this version, we sample both users and items.
