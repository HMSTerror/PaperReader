...
- 论文 PDF: [DCN V2 - Improved Deep & Cross Network and Practical Lessons for Web-scale Learning to Rank Systems.pdf](DCN V2 - Improved Deep & Cross Network and Practical Lessons for Web-scale Learning to Rank Systems.pdf)
- 下载来源: https://arxiv.org/pdf/2008.13535v2.pdf
- 元数据来源: OpenAlex http://arxiv.org/abs/2008.13535v2
- 年份/来源: 2020 / arXiv
- 作者: Ruoxi Wang、Rakesh Shivanna、Derek Z. Cheng 等

## 摘要

本文是 **点击率预估 (Click-Through Rate Prediction) 与学习排序 (Learning to Rank)** 方向的重要论文。论文围绕推荐系统中“如何从稀疏、噪声化、动态变化的用户行为中学习可靠排序信号”这一核心问题展开，试图用新的模型结构或训练目标改善传统协同过滤与深度推荐的不足。OpenAlex 未返回稳定被引次数。 对初学者而言，这篇文章的价值不只在于提出一个模型名称，更在于它明确回答了推荐系统研究中一个常见问题：模型应当学习什么形式的用户偏好，以及这种偏好如何转化为可评测的 Top-N 推荐结果。
...
**Materials**: 可提取文本中识别到的数据集或材料包括：MovieLens、Criteo。可提取指标包括：HR、AUC、Logloss、MAP、RMSE。可提取基线包括：DeepFM、DIN、DIEN。这些信息用于构造训练集、验证集和测试集，并评估模型在候选排序任务上的表现。
...
关于不足和未来方向，本报告只记录可从论文可提取文本中直接看到的作者表述，避免替作者补造局限。可提取线索如下：To capture effective feature crosses more accurately, a common remedy is to further increase model capacity through wider or deeper networks.；In addition, we propose a technique based on the Mixture-of-Expert architecture [19, 45] to further decompose the matrix into multiple smaller sub-spaces.；over, both DeepFM and xDeepFM require all the feature em- beddings to be of equal size, yet another limitation when applying to industrial data where the vocab sizes (sizes of cate
