...
- 论文 PDF: [Wide &amp; Deep Learning for Recommender Systems.pdf](Wide &amp; Deep Learning for Recommender Systems.pdf)
- 下载来源: https://arxiv.org/pdf/1606.07792v1.pdf
- 元数据来源: OpenAlex http://arxiv.org/abs/1606.07792v1
- 年份/来源: 2016 / arXiv
- 作者: Heng-Tze Cheng、Levent Koc、Jeremiah Harmsen 等
...
**Materials**: 可提取文本中识别到的数据集或材料包括：Netflix。可提取指标包括：HR、AUC、MAP、RMSE。可提取基线包括：Wide & Deep、DIN、DIEN。这些信息用于构造训练集、验证集和测试集，并评估模型在候选排序任务上的表现。
...
关于不足和未来方向，本报告只记录可从论文可提取文本中直接看到的作者表述，避免替作者补造局限。可提取线索如下：We have also open-sourced our implementation in TensorFlow.；We have also open-sourced our implementation in TensorFlow.；One limitation of cross-product trans- formations is that they do not generalize to query-item fea- ture pairs that have not appeared in the training data.
